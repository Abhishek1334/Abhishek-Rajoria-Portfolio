          ...
          <div className="w-full max-w-md my-2">
            ...
            <div className="bg-gradient-to-r from-amber-500/10 via-purple-500/5 to-amber-500/10 border-b border-white/10">
                <div className="p-4 flex flex-col gap-2">
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500/20 to-purple-500/20 flex items-center justify-center border border-amber-500/20 flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {project.githubUrl && (
                      <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="btn-ghost flex-1 px-3 py-2 text-xs flex items-center justify-center gap-2">
              ...
              {/* Image carousel */}
              <div className="relative flex flex-col items-center justify-center bg-gradient-to-br from-amber-500/5 to-purple-500/5 py-4">
                {project.media && project.media.length > 0 && (
                  <div className="relative w-full flex flex-col items-center">
                    <div className="w-full flex justify-center">
                      {project.media[currentSlide].type === 'video' ? (
                        <div className="relative w-full flex items-center justify-center bg-black/20">
                          <video src={project.media[currentSlide].url} className="max-h-48 max-w-full object-contain rounded-lg" controls preload="metadata" />
                        </div>
                      ) : (
                        <div className="relative w-full flex items-center justify-center cursor-pointer" onClick={() => setIsImageExpanded(true)}>
                          <img
                            src={project.media[currentSlide].url}
                            alt={project.media[currentSlide].alt}
                            className="max-h-48 max-w-full object-contain rounded-lg p-1 transition-transform duration-300"
                            draggable={false}
                          />
                    ...
                    {project.media.length > 1 && (
                      <>
                        <button onClick={handlePrevSlide} className="absolute left-2 top-1/2 -translate-y-1/2 bg-primary/80 text-primary-foreground border border-primary rounded-full shadow p-2 transition-colors hover:bg-primary hover:text-white z-10">
                          <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button onClick={handleNextSlide} className="absolute right-2 top-1/2 -translate-y-1/2 bg-primary/80 text-primary-foreground border border-primary rounded-full shadow p-2 transition-colors hover:bg-primary hover:text-white z-10">
                          <ChevronRight className="w-4 h-4" />
                        </button>
                        {/* Slide indicator */}
                        <div className="absolute bottom-2 left-1/2 -translate-x-1/2">
                          <div className="glass-card px-2 py-1 bg-black/60">
                            <span className="text-white text-xs font-medium">{currentSlide + 1} / {project.media.length}</span>
                          </div>
                        </div>
                      </>
                    )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, index) => (
                      <span key={index} className="px-2 py-1 text-xs font-medium rounded-full bg-primary/10 text-primary border border-primary/20">{tag}</span>
                    ))}
                  </div>
                </div>
                {/* Key Highlights */}
                <div>
                  ...
                  <div className="space-y-2">
                    {project.highlights.slice(0, 3).map((highlight, index) => (
                      <div key={index} className="flex items-start gap-3 text-sm text-foreground-muted">
                        <div className="w-2 h-2 rounded-full bg-amber-500 flex-shrink-0 mt-2" />
                        <span className="leading-relaxed">{highlight}</span>
                      </div>
                    ))}
                    {project.highlights.length > 3 && (
                      <div className="text-xs text-foreground-muted/70 italic">
                        +{project.highlights.length - 3} more highlights
                      </div>
                    )}
                  </div>
                </div>
                {/* --- More Details: Overview, Features, Tech Stack --- */}
                <div className="space-y-3">
                  <details open className="rounded-lg border border-border bg-muted/40">
                    <summary className="font-semibold text-primary py-2 px-2 cursor-pointer select-none">Overview</summary>
                    <div className="px-2 pb-2 text-sm text-foreground-muted">
                      {project.detailedContent.overview}
                    </div>
                  </details>
                  <details className="rounded-lg border border-border bg-muted/40">
                    <summary className="font-semibold text-primary py-2 px-2 cursor-pointer select-none">Features</summary>
                    <ul className="px-4 pb-2 list-disc text-sm text-foreground-muted space-y-1">
                      {project.detailedContent.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-2"><Zap className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" /><span>{feature}</span></li>
                      ))}
                    </ul>
                  </details>
                  <details className="rounded-lg border border-border bg-muted/40">
                    <summary className="font-semibold text-primary py-2 px-2 cursor-pointer select-none">Tech Stack</summary>
                    <div className="px-2 pb-2 flex flex-wrap gap-2">
                      {project.detailedContent.techStack.map((tech, idx) => (
                        <span key={idx} className="px-2 py-1 text-xs font-medium rounded-full bg-primary/10 text-primary border border-primary/20">{tech}</span>
                      ))}
                    </div>
                  </details>
                </div>
            ...
            <div className="bg-gradient-to-r from-secondary/10 via-primary/5 to-secondary/10 border-b border-border">
              <div className="p-4 sm:p-6">
                ...
                <div className="flex items-center gap-3">
                  {project.liveUrl && (
                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="btn-primary px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base flex items-center gap-2 hover:scale-105 transition-transform">
                  ...
                  {project.githubUrl && (
                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="btn-ghost px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base flex items-center gap-2 hover:scale-105 transition-transform">
